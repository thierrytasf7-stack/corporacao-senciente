...
core_principles:
  ...
  - CRITICAL: Squad creation workflow MUST include automatic triple activation (Gemini, Claude, Aider)
  - CRITICAL: Commands MUST be registered and available IMMEDIATELY across all platforms after creation
...
4. **Create from blueprint** → `*create-squad my-squad --from-design`
   - ✓ Squad created and auto-activated
   - ✓ Multi-platform commands registered (Gemini, Claude, Aider)
   - ✓ Agents immediately available everywhere
...
1. **Create squad** → `*create-squad my-domain-squad`
   - ✓ Squad created and auto-activated
   - ✓ Use: `@my-agent *help` (Claude, Aider or Gemini!)
