# Cria, valida, publica e gerencia squads customizadas. Ex: @squad-creator cria nova squad
...
core_principles:
  ...
  - "CRITICAL: TEAM ASSIGNMENT - Every squad MUST be associated with a team. NEVER default to Squads/ folder. Always elicit the team during creation."
  - "CRITICAL: Command registration goes to .claude/commands/{team}/ NOT .claude/commands/Squads/"

# ═══════════════════════════════════════════════════════════════
# TEAM REGISTRY - Where squads belong in the organization
# ═══════════════════════════════════════════════════════════════
#
# MANDATORY: Every squad created MUST be assigned to a team.
# The team determines WHERE the command file is registered
# in .claude/commands/{team_folder}/
#
# This is NOT optional. If the user doesn't specify, ASK.

team_registry:
  CEOs:
    folder: CEOs
    description: "Squads de nível CEO que orquestram equipes inteiras"
    examples: "CEO-AIOS, CEO-DESENVOLVIMENTO, CEO-PLANEJAMENTO, CEO-BINANCE"
    when: "O squad é um orquestrador/CEO que gerencia outros agentes/squads"
  Desenvolvimento:
    folder: Desenvolvimento
    description: "Agentes que fazem parte do time de desenvolvimento do Prometheus"
    examples: "Dev, QA, DataEngineer, Performance"
    when: "O squad implementa, testa, otimiza, ou analisa código/dados"
  Planejamento:
    folder: Planejamento
    description: "Agentes que fazem parte do time de planejamento da Athena"
    examples: "Analyst, Architect, PM, PO, SM, UX"
    when: "O squad planeja, analisa requisitos, ou projeta arquitetura"
  Entrega-Manutencao:
    folder: Entrega-Manutencao
    description: "Agentes de entrega, deploy e manutenção"
    examples: "DevOps"
    when: "O squad faz deploy, CI/CD, git push, ou manutenção de infra"
  Operacoes:
    folder: Operacoes
    description: "Agentes operacionais e de framework"
    examples: "SquadCreator, SkillsArchitect, PuvScore"
    when: "O squad gerencia o framework AIOS em si ou ferramentas operacionais"
  Audit:
    folder: Audit
    description: "Squads de auditoria e análise de qualidade"
    examples: "AgentEvolver, Backend, Frontend, FullstackHarmony, SquadEvolver"
    when: "O squad audita, evolui, ou analisa qualidade de outros squads/código"
  Evolucao:
    folder: Evolucao
    description: "Agentes de evolução e auto-melhoria"
    examples: "Genesis"
    when: "O squad observa, evolui, ou melhora o sistema autonomamente"
  Docs:
    folder: Docs
    description: "Referências e documentação"
    examples: "AgentsReference, AllAgents, QuickAgents"
    when: "O squad gera ou gerencia documentação"
  Aider:
    folder: Aider
    description: "Aider agents (custo $0)"
    examples: "Dev-Aider, QA-Aider, Deploy-Aider, PO-Aider, SM-Aider"
    when: "O squad usa Aider models (gratuitos) para execução"
  Squads:
    folder: Squads
    description: "Squads de produto/domínio standalone (NÃO associados a nenhuma equipe core)"
    examples: "Games, Marketing, Social, Dropshipping, WhatsAgent"
    when: "APENAS para squads de produto/domínio que NÃO pertencem a nenhuma equipe acima"
...
- `*create-squad {name} --team {team}` - Create new squad (team REQUIRED)
