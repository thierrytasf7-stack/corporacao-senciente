...
const fetcher = (url: string): Promise<ContextData> => fetch(url).then((r) => r.json());
    ...
    <div className="flex items-start gap-3 p-2 rounded-lg hover:bg-muted/50 transition-colors group">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mt-1 text-xs text-muted-foreground">
          ...
          <span suppressHydrationWarning>{new Date(file.lastModified).toLocaleDateString()}</span>
...
export function ContextPanel() {
  ...
  const { data: apiData, isLoading: apiLoading, error: apiError } = useSWR<ContextData>(
    !settings.useMockData ? '/api/context' : null,
    fetcher,
    { refreshInterval: 30000 }
  );
  const data = settings.useMockData ? MOCK_CONTEXT : apiData;

  if (apiLoading && !settings.useMockData) {
    return (
      <div className="flex h-full items-center justify-center">
        <div className="text-center">
          <Loader2 className="h-12 w-12 text-muted-foreground mx-auto mb-4 animate-spin" />
          <p className="text-lg font-semibold text-foreground">Loading Context...</p>
          <p className="mt-1 text-sm text-muted-foreground">
            Lendo arquivos de configuracao do projeto.
          </p>
        </div>
      </div>
    );
  ...
  if (apiError && !settings.useMockData) {
    return (
      <div className="flex h-full items-center justify-center">
        <div className="text-center">
          <XCircle className="h-12 w-12 text-red-500 mx-auto mb-4" />
          <p className="text-lg font-semibold text-foreground">Error Loading Context</p>
          <p className="mt-1 text-sm text-muted-foreground">
            Nao foi possivel ler os dados do projeto. Verifique se o servidor esta rodando.
          </p>
    ...
    return (
      <div className="flex h-full items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-lg font-semibold text-foreground">No Context Available</p>
          <p className="mt-1 text-sm text-muted-foreground">
            O Context mostra arquivos de configuracao, MCPs ativos e variaveis de ambiente do projeto.
          </p>
          <p className="mt-1 text-xs text-muted-foreground">
            Ative o Demo Mode nas Settings para visualizar dados de exemplo.
          </p>
        </div>
      </div>
    );
