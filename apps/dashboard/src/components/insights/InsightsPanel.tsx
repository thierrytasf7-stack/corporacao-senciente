...
import { TrendingUp, TrendingDown, Clock, AlertTriangle, CheckCircle2, Activity, Loader2, Brain } from 'lucide-react';
...
export function InsightsPanel() {
  ...
  const { data: apiData, error, isLoading } = useSWR<InsightsMetrics & { aiInsight?: string }>(
    settings.useMockData ? null : '/api/insights',
    fetcher,
    { refreshInterval: 60000 }
  );

  const data: (InsightsMetrics & { aiInsight?: string }) | null = settings.useMockData
    ? MOCK_INSIGHTS
    : apiData ?? null;
    ...
    return (
      <div className="flex h-full items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-lg font-semibold text-foreground">No Insights Available</p>
          <p className="mt-1 text-sm text-muted-foreground">
            {error ? 'Erro ao carregar insights. Verifique o servidor.' : 'Insights mostra metricas de produtividade dos agentes, stories processadas e tendencias.'}
          </p>
          <p className="mt-1 text-xs text-muted-foreground">
            Ative o Demo Mode nas Settings para visualizar analytics de exemplo.
          </p>
        </div>
      </div>
    );
  ...
  const velocityChange = data.velocity.previous > 0
    ? ((data.velocity.current - data.velocity.previous) / data.velocity.previous * 100).toFixed(0)
    : data.velocity.current > 0 ? '100' : '0';
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
        ...
        {data.aiInsight && (
          <section>
            ...
            <div className="p-4 rounded-lg border border-border bg-card">
              <p className="text-sm whitespace-pre-wrap">{data.aiInsight}</p>
            </div>
          </section>
        )}
