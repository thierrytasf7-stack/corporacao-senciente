...
export function TerminalStream({ terminalId, onClose, className }: TerminalStreamProps) {
  const terminal = useTerminalStore((state) => state.terminals[terminalId]);
  ...
  // Connect to SSE - stable callback using store.getState()
  const connect = useCallback(() => {
    const store = useTerminalStore.getState();
    const t = store.terminals[terminalIdRef.current];
    if (!t) return;
    ...
    store.setTerminalStatus(terminalIdRef.current, 'connecting');

    const url = `/api/logs?agent=${t.agentId}`;
    const eventSource = new EventSource(url);
    eventSourceRef.current = eventSource;

    eventSource.onopen = () => {
      useTerminalStore.getState().setTerminalStatus(terminalIdRef.current, 'connected');
    };

    eventSource.onerror = () => {
      useTerminalStore.getState().setTerminalStatus(terminalIdRef.current, 'error', 'Connection failed');
      eventSource.close();
    ...
    eventSource.addEventListener('log:line', (event: MessageEvent) => {
      try {
        ...
        useTerminalStore.getState().appendLine(terminalIdRef.current, {
    ...
    eventSource.addEventListener('log:init', () => {
      useTerminalStore.getState().clearTerminal(terminalIdRef.current);
    });
    ...
    eventSource.addEventListener('log:error', (event: MessageEvent) => {
      try {
        ...
        useTerminalStore.getState().setTerminalStatus(terminalIdRef.current, 'error', error);
      } catch {
        useTerminalStore.getState().setTerminalStatus(terminalIdRef.current, 'error', 'Unknown error');
      }
    });
  ...
  // Disconnect - stable callback
  const disconnect = useCallback(() => {
    if (eventSourceRef.current) {
      eventSourceRef.current.close();
      eventSourceRef.current = null;
    }
    useTerminalStore.getState().setTerminalStatus(terminalIdRef.current, 'disconnected');
  }, []); // No dependencies - uses refs and getState()
