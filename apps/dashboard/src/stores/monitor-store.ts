...
export const selectCurrentCommand = (state: MonitorState): { name: string; status: 'running' | 'complete' | 'error'; startedAt: number } | null => {
  ...
  // Check most recent completed tool
  const lastPost = state.events.find((e) => e.type === 'PostToolUse');
  if (lastPost) {
    return {
      name: lastPost.tool_name || 'unknown',
      status: lastPost.is_error ? 'error' : 'complete',
      startedAt: lastPost.timestamp - (lastPost.duration_ms || 0),
    };
  }
...
export const selectActiveAgent = (state: MonitorState): { id: string; name?: string } | null => {
  const recentEvent = state.events.find((e) => e.aios_agent);
  if (recentEvent?.aios_agent) {
    return { id: recentEvent.aios_agent };
  }
  return null;
};
