...
Server runs on `http://localhost:21302` by default.
...
Navigate to `http://localhost:21300/monitor` to see real-time events.
...
| -------------- | --------------------------- | -------------------- |
| `DIANA_MONITOR_PORT` | `21302`                      | Server port          |
...
| Variable                  | Default                 | Description                     |
| ------------------------- | ----------------------- | ------------------------------- |
| `AIOS_MONITOR_URL`        | `http://localhost:21302` | Monitor server URL              |
...
1. Ensure `NEXT_PUBLIC_MONITOR_WS_URL` is set in Dashboard `.env`:
   ...
   ```
   NEXT_PUBLIC_MONITOR_WS_URL=ws://localhost:21302/stream
   ```
...
```bash
# Manual cleanup
curl -X POST http://localhost:21302/cleanup?hours=12
```
