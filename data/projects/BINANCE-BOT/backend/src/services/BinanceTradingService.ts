...
export interface HedgeOrderRequest extends BinanceOrderRequest {
  isHedge?: boolean;
}
...
export class BinanceTradingService {
  ...
  private strategyLoadBalancer: StrategyLoadBalancer;
  ...
  constructor(
    ...
    this.strategyLoadBalancer = new StrategyLoadBalancer(maxGlobalTrades);
  ...
  private setupInterceptors(): void {
    this.client.interceptors.request.use(
      ...
      (error) => {
        logger.error('Binance trading request error:', {
  ...
  private createSignedParams(params: Record<string, any> = {}): string {
    // Corrigir timestamp - reduzir 10 segundos para evitar erro de sincronização
  ...
  async placeMarketOrder(
    request: BinanceOrderRequest & { strategyId?: string }
  ): Promise<BinanceOrderResponse> {
    try {
      // Verifica se pode executar trade para esta estratégia
      if (request.strategyId) {
        if (!this.strategyLoadBalancer.canStartNewTrade(request.strategyId, request.symbol)) {
          throw new Error('Strategy trade limit reached or strategy disabled');
        }
        this.strategyLoadBalancer.incrementTrades(request.strategyId);
      }
   ...
   * Place a hedge order
   */
  async placeHedgeOrder(request: HedgeOrderRequest): Promise<BinanceOrderResponse> {
    try {
      logger.info('Placing hedge order:', request);
      
      const validation = await this.validateOrderParams(request);
      if (!validation.isValid) {
        throw new Error(`Invalid hedge order parameters: ${validation.errors.join(', ')}`);
      }

      // Adicionar identificador para ordens de hedge
      request.newClientOrderId = `HEDGE_${Date.now()}`;

      const response = await this.placeMarketOrder(request);
      ...
      logger.error('Failed to place hedge order:', error);
      throw new Error(`Failed to place hedge order: ${error.message}`);
    }
  }
   ...
   * Get hedge orders
   */
  async getHedgeOrders(symbol?: string): Promise<any[]> {
    try {
      const allOrders = await this.getOpenOrders(symbol);
      return allOrders.filter(order => order.clientOrderId?.startsWith('HEDGE_'));
    } catch (error) {
      logger.error('Failed to get hedge orders:', error);
      throw error;
    }
  }

  /**
   * Regista uma nova estratégia no load balancer
   */
  registerStrategy(strategy: Strategy): void {
    this.strategyLoadBalancer.registerStrategy(strategy);
  }

  /**
   * Remove uma estratégia do load balancer
   */
  unregisterStrategy(strategyId: string): void {
    this.strategyLoadBalancer.unregisterStrategy(strategyId);
  }

  /**
   * Obtém as alocações atuais das estratégias
   */
  getStrategyAllocations(): StrategyAllocation[] {
    return this.strategyLoadBalancer.getAllocations();
  }
  ...
  updateStrategyStatus(strategyId: string, enabled: boolean): void {
    this.strategyLoadBalancer.updateStrategyStatus(strategyId, enabled);
  }

  /**
   * Finaliza uma trade para uma estratégia
   */
  finalizeTrade(strategyId: string): void {
    this.strategyLoadBalancer.decrementTrades(strategyId);
  }
}
