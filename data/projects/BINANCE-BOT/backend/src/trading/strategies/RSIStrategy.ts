...
import { BaseStrategy, SignalResult, StrategyParams } from './BaseStrategy';
  ...
  protected async checkSignals(): Promise<SignalResult> {
    ...
    // Exit logic
      ...
      if (currentRSI > this.oversoldLevel + 10 || currentRSI > this.overboughtLevel) {
        signal = 'SELL';
        strength = Math.max(0, (currentRSI - this.oversoldLevel) / (this.overboughtLevel - this.oversoldLevel) * 100);
      ...
      if (currentRSI < this.overboughtLevel - 10 || currentRSI < this.oversoldLevel) {
        ...
        strength = Math.max(0, (this.overboughtLevel - currentRSI) / (this.overboughtLevel - this.oversoldLevel) * 100);
