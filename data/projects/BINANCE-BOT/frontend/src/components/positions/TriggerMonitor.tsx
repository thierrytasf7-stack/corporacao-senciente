...
const TriggerMonitor: React.FC<TriggerMonitorProps> = ({ executions, onSellExecution, onRefreshData }) => {
    ...
    const fetchPrices = async (symbols: string[]) => {
        ...
        try {
            ...
            const batchUrl = `http://127.0.0.1:23231/api/v1/binance/prices?symbols=${encodeURIComponent(symbols.join(','))}`;
            ...
            const results = await Promise.all(
                symbols.map(async (sym) => {
                    try {
                        const r = await fetch(`http://127.0.0.1:23231/api/v1/binance/price/${sym}`);
                        if (!r.ok) return null;
