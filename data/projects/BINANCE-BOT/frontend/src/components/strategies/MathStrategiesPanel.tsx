...
const MathStrategiesPanel: React.FC = () => {
  ...
  const fetchStrategies = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://127.0.0.1:23231/api/v1'}/math-strategies`);
  ...
  const toggleStrategy = async (strategyId: string) => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://127.0.0.1:23231/api/v1'}/math-strategies/${strategyId}/toggle`, {
        method: 'POST',
      });
      const data = await response.json();
