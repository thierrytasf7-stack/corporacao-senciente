# [STORY-20260212232213-4] Reduzir Tempo de Startup do PM2 Ecosystem
> **Status:** REVISADO
...
> **Dificuldade:** LOW
> **Prioridade:** Alta
> **Criado em:** 2026-02-12 23:22:13

## Contexto
Startup simultaneo de todos os processos causa picos de CPU que podem travar maquinas com recursos limitados. Sequenciar a inicializacao garante que dependencias estejam prontas e reduz falhas de timeout na inicializacao.

## Objetivo
O ecosystem.config.js inicia todos os processos simultaneamente causando picos de CPU. Implementar startup sequencial com delay entre processos e health check antes de iniciar o proximo worker.
...
## Output Esperado
- Script scripts/startup-sequencer.js que inicia processos PM2 em ordem com delay
- Health check simples (porta TCP ou processo ativo) entre cada inicializacao
- Documentacao em comentarios do script com tempo de startup antes vs depois
- Integracao com Start-Diana-Native.bat como alternativa ao pm2 start direto

## Acceptance Criteria
- [?] Processos iniciam com delay configuravel entre eles
- [?] Health check basico antes de iniciar proximo processo
- [?] Tempo total de startup documentado antes e depois
- [?] Nenhum processo falha por dependencia nao disponivel

## Constraints
- Usar Node.js stdlib (child_process, net) - sem dependencias externas
- Delay padrao de 3 segundos entre processos, configuravel via variavel
- Manter ecosystem.config.js inalterado - sequencer e complementar, nao substituto
...
## Instrucoes
1. Analisar dependencias entre processos no ecosystem.config.js. 2. Criar script scripts/startup-sequencer.js com ordem e delays. 3. Adicionar health check simples (porta aberta ou processo rodando). 4. Medir e documentar tempo de startup antes e depois da otimizacao.
...
const STARTUP_ORDER = [
  { name: 'dashboard-ui', port: 3000, delay: 3000 },
  { name: 'guardian-hive', port: null, delay: 2000 },
  { name: 'maestro', port: null, delay: 2000 },
];

async function waitForPort(port, timeout = 10000) {
  return new Promise((resolve) => {
    const client = net.connect({ port }, () => { client.end(); resolve(true); });
...
## Review Results
> Reviewed at: 2026-02-12T23:29:55.508564
> Verdict: FAIL
...
## Review Results
> Reviewed at: 2026-02-12T23:31:13.643873
> Verdict: FAIL
