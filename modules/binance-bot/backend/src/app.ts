...
export class App {
  ...
  private setupRoutes(): void {
    ...
    // Logs routes (for saving console logs)
    try {
      this.app.use('/api/logs', logsRouter);
      this.app.use('/api/v1/logs', logsRouter); // Adicionar rota v1 para compatibilidade
    } catch (error) {
      console.warn('Logs routes not available:', error);
    }
