  ...
  private mergeWithEnvironmentVariables(baseConfig: Partial<AppConfig>, environment: string): AppConfig {
    return {
      ...
      binance: {
        ...
        useTestnet: true, // Forçar Testnet
        apiKey: process.env.BINANCE_API_KEY || baseConfig.binance?.apiKey,
        secretKey: process.env.BINANCE_SECRET_KEY || baseConfig.binance?.secretKey,
      ...
      server: {
        port: parseInt(process.env.PORT || '') || baseConfig.server?.port || 21341,
        host: process.env.HOST || baseConfig.server?.host || 'localhost',
        corsOrigins: process.env.CORS_ORIGINS?.split(',') || baseConfig.server?.corsOrigins || ['http://localhost:21340'],
