...
import spotRotativeAnalysisRoutes from '@/routes/spotRotativeAnalysis';
...
export class ApiGateway {
  ...
  private setupRoutes(): void {
    ...
    this.routes = [
      ...
      {
        path: '/binance',
        router: binanceRoutes,
        requireAuth: false, // Para uso pessoal, sem autenticação
        rateLimit: { maxRequests: 500, windowMs: 15 * 60 * 1000 }, // 500 requests per 15 minutes
        description: 'Binance API integration endpoints (real data)'
      },
      {
        path: '/sell',
        router: sellRoutes,
        requireAuth: false, // Para uso pessoal, sem autenticação
        rateLimit: { maxRequests: 100, windowMs: 15 * 60 * 1000 }, // 100 requests per 15 minutes
        description: 'Sell execution endpoints'
      },
      {
        path: '/spot-rotative-analysis',
        router: spotRotativeAnalysisRoutes,
        requireAuth: false, // Para uso pessoal, sem autenticação
        rateLimit: { maxRequests: 200, windowMs: 15 * 60 * 1000 }, // 200 requests per 15 minutes
        description: 'Spot rotative analysis endpoints'
      }
    ];
  }
